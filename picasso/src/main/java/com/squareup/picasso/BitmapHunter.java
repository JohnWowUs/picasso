...
class BitmapHunter implements Runnable {
  ...
  static Bitmap transformResult(Request data, Bitmap result, int exifOrientation) {
	...
	// Recalulate dimensions after rotation
        double tmpHeight = (double)targetHeight;
        double tmpWidth  = (double)targetWidth;
	double cosRotation = Math.cos(Math.toRadians(targetRotation));
	double sinRotation = Math.sin(Math.toRadians(targetRotation));
        targetHeight = (int)Math.floor((tmpHeight * cosRotation) + (tmpWidth * sinRotation)) ;
        targetWidth  = (int)Math.floor((tmpHeight * sinRotation) + (tmpWidth * cosRotation)) ;
      }
